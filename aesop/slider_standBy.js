	...
	SwiperB.prototype = {
		init: function() {
			...
			resImgArr.forEach((item, index) => {
				str += `<a href="${resImgArr[index].url}"><div class="swiper-slide${this.clsSuffix}" style="width: ${this.imgWidth}px;background:url(${resImgArr[index].imgPath}) center top / contain;"></div></a>`;
			});
		...
		eventBind: function() {	
			let that = this;
            this.mainDom.addEventListener('prev', ()=>{
                console.log("getPrev")
                that.prevSlider(that.aniTime);
            })

            this.mainDom.addEventListener('next', ()=>{
                console.log("getNext")
