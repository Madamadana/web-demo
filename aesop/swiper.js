	...
	* gap 图片之间间隔
	*/
	const bg = document.querySelector(".swiper-main-bg");
	...
	function Swiper(obj) {
		...
		this.imgWidth = this.containerWidth * obj.imgWidth;
	...
	Swiper.prototype = {
		init: function() {
			...
			resImgArr.forEach((item, index) => {
				str += `<a href="${resImgArr[index].url}"><div class="swiper-slide${this.clsSuffix}" style="width: ${this.imgWidth}px;background:url(${resImgArr[index].imgPath}) center bottom / contain no-repeat;"></div></a>`;
			});
			this.mainDom.innerHTML = str;
			this.setScale();
			//this.timer = setInterval(this.nextSlider.bind(this, this.aniTime), this.intervalTime);
			setProduct(this.nowIndex-2);
		},
		setScale: function() {
			// 堆叠式
			if (this.gap < 0) {
		...
		prevSlider: function(aniTime) {
			...
			if (this.imgArr.length ===2) {
				...
				this.nowIndex--;
				this.mainDom.style.transition = `left ${aniTime/1000}s ease-out`
			...
			setProduct(this.nowIndex>1 ? this.nowIndex-2 : this.nowIndex+1);
		},
		nextSlider: function(aniTime) {
			bg.dispatchEvent(eventNext);
			if (this.imgArr.length ===2) {
				this.nowIndex = this.nowIndex ? 0 : 1;
				this.setScale()
			} else if (this.imgArr.length ===1) {
		...
		eventBind: function() {	
			...
			this.mainDom.addEventListener('touchend', function(e) {
				if (!that.mainDom.style.transition) {
					that.mainDom.style.transition = `left ${that.aniTime / 1000}s ease-out`
				}
